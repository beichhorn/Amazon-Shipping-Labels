...
def getSignatureKey(key, dateStamp, regionName, serviceName):
    key = 'AWS4' + key
    kDate    = sign(bytes(key, "ascii"), dateStamp, True)
...
poNumber  = sys.argv[1][0:9]

# Get selling party from text file
f = open("/home/amazon/sellingParty.txt", "r")
sellingParty = (f.read())
f.close()

# Get ship from party from text file
f = open("/home/amazon/shipFromParty.txt", "r")
shipFromParty = (f.read())
f.close()

# request_parameters = '{"sellingParty": {"partyId": "YORAL"},"shipFromParty": {"partyId": "ACLF"}}' 
request_parameters = '{"sellingParty": {"partyId": ' + sellingParty + '},"shipFromParty": {"partyId": ' + shipFromParty + '}}'
canonical_uri = "/vendor/directFulfillment/shipping/2021-12-28/shippingLabels/"+poNumber.strip()
